...
import {
  deleteImageByExperienceIdAndURL,
...
import { getOwnedExperiences } from "../models/experiences-model.js";
...
export const uploadMultipleExperienceImages = async (req, res, next) => {
  const SUBDIR = "timeslots";
  ...
  try {
    ...
    const urls = result.map((r) => [r.url, req.params.experience_id]);
    await uploadTimeSlotImagesExperience(undefined, urls);
...
export const deleteExperienceImageByUrl = async (req, res, next) => {
  try {
    const { experience_id } = req.params;
    const { image_url } = req.body;

    const r = await getOwnedExperiences(req.user.id);

    const ownedExperiences = r.map((t) => t.id);

    const isOwner = ownedExperiences.some(
      (ownedId) => String(ownedId) === String(experience_id),
    );
    ...
    const deletedCount = await deleteImageByExperienceIdAndURL(
      experience_id,
      image_url,
    );
